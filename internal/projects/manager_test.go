...
func TestAddClonesAbsentPathAndRegistersProject(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"chown -R " + claudecode.User + ":torio-projects -- " + testPath,
		"find " + testPath + " -type d -exec chmod g+s",
		"sudo -n -u " + claudecode.User + " -- git config --global --add safe.directory " + testPath,
...
func TestAddProvisionsDeployKeyWhenSSHRemoteUnreadable(t *testing.T) {
	...
	assertKind(t, err, KindAuth)
	for _, fragment := range []string{
		"mkdir -p -m 0700 " + claudecode.Home + "/.ssh " + claudecode.Home + "/.ssh/torio",
...
func TestAddReusesAnExistingKeyAndProvisionsGuestGitConfig(t *testing.T) {
	...
	if !g.saw("sudo -n -u " + claudecode.User + " -- git config --global includeIf.gitdir:") {
...
func TestAddAdoptsCompliantCheckoutThatIsNotRegisteredYet(t *testing.T) {
	...
	if report.Cloned || !report.Adopted || !report.Registered {
		t.Fatalf("report = %#v, want an adopted, recorded checkout", report)
	}
	if g.saw("git clone") {
...
func TestAddRefusesConflictingCheckouts(t *testing.T) {
	cases := []struct {
		...
		{"nested repository", func(f *fakeGuest) { f.topLevel = claudecode.WorkspacePath }, []string{"Git repository"}},
...
func TestDerivePathContainsEveryIDUnderTheWorkspaceRoot(t *testing.T) {
	for _, id := range []string{"../etc", "a/b", "..", ".", "", "a/../../b"} {
		if _, err := derivePath(claudecode.WorkspacePath, id); err == nil {
			t.Errorf("derivePath(claudecode.WorkspacePath, %q) error = nil, want a containment failure", id)
		}
	}
	got, err := derivePath(claudecode.WorkspacePath, testID)
	if err != nil || got != testPath {
		t.Fatalf("derivePath(claudecode.WorkspacePath, %q) = %q, %v; want %q, nil", testID, got, err, testPath)
	}
}
...
func TestAddConfigWriteFailureKeepsTheCheckout(t *testing.T) {
	g := readyFake()
	r := emptyRegistry()
	...
	for _, want := range []string{"checkout_retained", "rerun_finishes"} {
...
// the checkout is still compliant, so the rerun adopts it.
func TestAddRerunFinishesAfterAConfigWriteFailure(t *testing.T) {
	...
	if !report.Adopted || !report.Registered {
		t.Fatalf("report = %#v, want an adopted, recorded checkout", report)
	}
...
func TestAddFailsClosedOnTruncatedGuestOutput(t *testing.T) {
	g := readyFake()
	g.truncateOn = "git -C " + testPath + " rev-parse"
...
func TestRemoveKeepsTheCheckout(t *testing.T) {
	g := attachedFake()
	r := registryWith(testProject())
	...
	if !report.CheckoutRetained || report.CheckoutPath != testPath {
		t.Fatalf("report = %#v, want a retained checkout", report)
	}
...
// An interrupted removal always stops with the config entry still present, so a
// rerun finishes the write.
func TestRemoveRerunFinishesAnInterruptedRemoval(t *testing.T) {
	...
	assertKind(t, err, KindConfigWrite)
	if !slices.Contains(report.Notes, "config_entry_retained") {
		t.Fatalf("report = %#v, want a retained config entry", report)
	}
	...
	if _, err := m.Remove(context.Background(), testID); err != nil {
...
func TestListReportsDerivedPathsSortedByID(t *testing.T) {
	...
	want := []Project{
		...
		{ID: "zeta", DisplayName: "Zeta", Remote: "git@github.com:owner/zeta.git", Path: claudecode.WorkspacePath + "/zeta"},
	}
...
func TestShowReportsAnAbsentCheckout(t *testing.T) {
	...
	for _, want := range []string{"checkout_absent"} {
		if !slices.Contains(report.Issues, want) {
			t.Errorf("issues = %v, want %q", report.Issues, want)
		}
	}
...
func TestShellPreflightRefusesADriftedCheckout(t *testing.T) {
	for _, tc := range []struct {
		...
		{"shared permissions", func(f *fakeGuest) { f.mode = "755"; f.group = claudecode.User }, "shared_permissions_missing"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			g := shellFake()
			tc.mutate(g)
...
func TestAddFromBundleCarriesTheRepositoryInWithoutANetwork(t *testing.T) {
	...
	if g.copies[0].guest != testBundleStaging || g.copies[0].home != claudecode.Home {
	...
	for _, fragment := range []string{
		...
		// like to Git.
		"chown -R " + claudecode.User + ":" + claudecode.User + " " + testBundleStaging,
...
func TestAddFromBundleHandsTheCarriedFileToTheAgentBeforeCloning(t *testing.T) {
	...
	g.setFailure("chown -R "+claudecode.User, 1)
	bundle := writeTestBundle(t)
