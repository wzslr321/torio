...
// The small parsers this package needs. They are local rather than shared with
// the other backend because every one of them decides something a check then
// fails closed on, and a shared helper that grew a lenient mode for one caller
// would quietly change what the others prove. The version parser below is the
// case in point: the two backends print their marker at opposite ends of the
// line, so one parser serving both would accept either program's output as
...
// fail for the wrong reason, which is indistinguishable from the outside from
// the download being wrong.
