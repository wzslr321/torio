...
# Codex runs it from the managed configuration as one of its hooks, so it runs as
# the agent identity and writes into that identity's own home. Root ownership
# buys the same thing it buys for the configuration itself: the agent cannot
...
# list. Codex passes a JSON document on standard input carrying the session id
# beside fields that contain agent-written prose. jq extracts only that bounded
# identifier; neither the rest of the payload nor any string from it reaches the
# marker.
set -euo pipefail
umask 077
...
# The hook document has prose-bearing fields, but the session id is a bounded
# identifier. The filter emits that field alone and rejects anything outside the
# alphabet the marker schema accepts.
...
# Two sessions can fire hooks concurrently. The fixed lock serializes updates to
# the one fixed marker path, avoiding both a lost update and any need for the
...
# fired it, so the ancestor is the answer. If Codex changes what a session runs
# as, no ancestor matches and the hook fails closed: a per-session wait without a
# live process cannot be ranked below liveness.
waiting_pid=0
if [ "$action" = 'set' ]; then
  p=$$
  while [ -n "$p" ] && [ "$p" -gt 1 ]; do
...
# afterwards: the reader refuses a marker anyone but its owner could write, and a
# window in which that is true is a window in which the marker is ignored.
