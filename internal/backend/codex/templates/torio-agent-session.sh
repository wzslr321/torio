...
#
# The host supplies exactly one value, the project path, and it is validated here
# as well as there. The host is a caller, not a trusted input source, and the
# command this helper runs is a constant below rather than something the caller
# composed.
set -euo pipefail
...
# -H sets HOME to the agent identity's own home, so its credential, configuration
# and skills resolve. The working directory is inherited, which is how the agent
