...
func TestExportDryRunBuildsManifestWithoutTransferringContent(t *testing.T) {
	destination := filepath.Join(t.TempDir(), "backup")
	g := initializedFake()
	g.exportFiles = map[string]string{
		"notes/decision.md":       "# Decision",
		"attachments/diagram.png": "png",
	}

	report, err := New(g).Export(context.Background(), ExportOptions{
	...
	if len(g.copies) != 0 {
		t.Fatalf("dry-run transferred working-tree content: %#v", g.copies)
	}
...
func TestExportRefusesExistingDestinationWithoutMutation(t *testing.T) {
	parent := t.TempDir()
	destination := filepath.Join(parent, "existing")
	if err := os.Mkdir(destination, 0o700); err != nil {
		t.Fatalf("Mkdir: %v", err)
	}
	writeHostTransferFile(t, destination, "keep.md", "keep", 0o600)

	g := initializedFake()
	g.exportFiles = map[string]string{"note.md": "note"}
	_, err := New(g).Export(context.Background(), ExportOptions{Destination: destination})
	if err == nil {
		t.Fatal("Export accepted an existing destination")
	}
	content, readErr := os.ReadFile(filepath.Join(destination, "keep.md"))
	if readErr != nil || string(content) != "keep" {
		t.Fatalf("existing destination changed: content=%q err=%v", content, readErr)
	}
	if len(g.copies) != 0 {
		t.Fatalf("existing destination triggered transfer: %#v", g.copies)
	}
}

func TestExportCopiesVerifiesAndAtomicallyPromotesWorkingTree(t *testing.T) {
	destination := filepath.Join(t.TempDir(), "backup")
	g := initializedFake()
	g.exportFiles = map[string]string{
		"notes/decision.md":       "# Decision",
	...
	g.copyFromHook = func(hostDestination string) error {
		for rel, content := range g.exportFiles {
			writeHostTransferFile(t, hostDestination, rel, content, 0o600)
		}
		return nil
	}

	report, err := New(g).Export(context.Background(), ExportOptions{Destination: destination})
	if err != nil {
		t.Fatalf("Export: %v", err)
	}
	if report.FinalPath != destination || report.Files != 2 || report.ManifestSHA256 == "" {
		t.Fatalf("report = %#v", report)
	}
	if len(g.copies) != 1 || g.copies[0].direction != "from_guest" {
		t.Fatalf("copies = %#v", g.copies)
	}
	if _, err := os.Stat(filepath.Join(destination, "notes", "decision.md")); err != nil {
		t.Fatalf("exported note missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(destination, "torio-brain-manifest.json")); err != nil {
		t.Fatalf("export manifest missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(destination, ".git")); !os.IsNotExist(err) {
		t.Fatalf("working-tree-only export contains .git, stat err = %v", err)
	}
}

func TestExportTransferFailureLeavesDestinationAbsent(t *testing.T) {
	destination := filepath.Join(t.TempDir(), "backup")
	g := initializedFake()
	g.exportFiles = map[string]string{"note.md": "note"}
	g.copyFromErr = context.DeadlineExceeded

	_, err := New(g).Export(context.Background(), ExportOptions{Destination: destination})
	if err == nil {
		t.Fatal("Export transfer failure returned nil")
	}
	if _, statErr := os.Stat(destination); !os.IsNotExist(statErr) {
		t.Fatalf("failed export left destination behind, stat err = %v", statErr)
	}
	if !g.saw("rm -rf -- " + exportStagingPath) {
		t.Fatalf("failed export did not clean guest staging: %#v", g.calls)
	}
}

func TestExportRejectsGuestHardlinksBeforeContentTransfer(t *testing.T) {
	destination := filepath.Join(t.TempDir(), "backup")
	g := initializedFake()
	g.exportFiles = map[string]string{"note.md": "note"}
	g.exportHardlink = true

	_, err := New(g).Export(context.Background(), ExportOptions{Destination: destination})
	if err == nil {
		t.Fatal("Export accepted a hardlinked Brain file")
	}
	if len(g.copies) != 0 {
		t.Fatalf("hardlink preflight transferred content: %#v", g.copies)
	}
	if _, statErr := os.Stat(destination); !os.IsNotExist(statErr) {
		t.Fatalf("hardlink rejection created destination, stat err = %v", statErr)
	}
}

func TestExportRefusesToOverwriteAWorkingTreeFileWithItsManifest(t *testing.T) {
	destination := filepath.Join(t.TempDir(), "backup")
	g := initializedFake()
	g.exportFiles = map[string]string{
		"note.md":                   "note",
		"torio-brain-manifest.json": "user data",
	}

	_, err := New(g).Export(context.Background(), ExportOptions{Destination: destination})
	if err == nil {
		t.Fatal("Export accepted a working-tree file reserved for its manifest")
	}
	if len(g.copies) != 0 {
		t.Fatalf("reserved manifest collision transferred content: %#v", g.copies)
	}
	if _, statErr := os.Stat(destination); !os.IsNotExist(statErr) {
		t.Fatalf("manifest collision created destination, stat err = %v", statErr)
	}
}
